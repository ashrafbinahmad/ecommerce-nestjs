...
export class CustomerService {
  ...
  async customerSignup(dto: CustomerSignupDto): Promise<Tokens> {
    ...
    const tokens = await this.getTokens(newUser.id, newUser.email, 'CUSTOMER');
    return tokens;
  }

  async customerLogin(dto: LoginCredentials): Promise<Tokens> {
    ...
    const tokens = await this.getTokens(user.id, user.email, 'CUSTOMER');
    return tokens;
  }
  ...
  async refreshTokens(rtDto: RefreshTokensDto): Promise<Tokens> {
    ...
    return this.getTokens(payload?.sub, payload?.email, 'CUSTOMER');
  }
